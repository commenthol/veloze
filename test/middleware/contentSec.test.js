...
describe('middleware/csp', function () {
  describe('buildCsp', function () {
    ...
    it('shall build csp header value from options', function () {
      ...
      assert.equal(
        result,
        "default-src *; connect-src *; font-src *; frame-src *; img-src *; manifest-src *; media-src *; object-src *; prefetch-src *; script-src *; script-src-elem *; script-src-attr 'none'; style-src *; worker-src *; base-uri *; sandbox allow-forms allow-same-origin allow-scripts allow-top-navigation allow-popups allow-pointer-lock; form-action *; frame-ancestors *; report-to abcd; report-uri /csp-report"
      )
    })
  ...
  describe('contentSec', function () {
    ...
    it('should return content-security-policy-report-only header', function () {
      ...
      app.get(
        '/',
        contentSec({ csp: { reportOnly: true, 'report-uri': '/report-url' } }),
        end
      )
      return supertest(app.handle)
        .get('/')
        ...
        .expect(
          shouldHaveHeaders({
            ...
            'content-security-policy-report-only':
              "default-src 'self'; font-src 'self' https: data:; img-src 'self' data:; object-src 'none'; script-src 'self'; script-src-attr 'none'; style-src 'self' 'unsafe-inline' https:; base-uri 'self'; form-action 'self'; frame-ancestors 'self'; report-uri /report-url; upgrade-insecure-requests",
            'cross-origin-embedder-policy': 'require-corp',
            'cross-origin-opener-policy': 'same-origin',
            'cross-origin-resource-policy': 'same-origin',
            'referrer-policy': 'no-referrer',
    ...
    it('shall report a csp violation', function (done) {
      ...
      const cspViolation = {
        'csp-report': {
          ...
          'original-policy':
            "default-src 'self'; script-src-elem 'unsafe-inline'; style-src 'unsafe-inline'; report-uri http://localhost/csp-report",
          referrer: '',
          'status-code': 200,
